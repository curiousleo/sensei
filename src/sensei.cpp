...
void display(const Sudoku &sudoku) {
    ...
    // Find padding width
    for (Sudoku::const_iterator cell_it = sudoku.begin();
    ...
    for (Sudoku::const_iterator cell_it = sudoku.begin();
         cell_it != sudoku.end(); ++cell_it) {
        line = "";
        for (Values::const_iterator value_it = cell_it->begin();
...
bool solve(Sudoku &sudoku) {
    ...
    for (Values::const_iterator guess_it = sudoku[min_cell_i].begin();
 ...
 * from the cell's peers. */
#ifndef NDEBUG
    if (std::find(sudoku[cell_i].begin(), sudoku[cell_i].end(), val) ==
        sudoku[cell_i].end())
        throw std::range_error(
            "Assignation not possible: value not in set of possible values");
    if (val > 9)
    ...
    for (std::set<tiny>::const_iterator it = peers[cell_i].begin();
...
void eliminate(Sudoku &sudoku) {
    ...
    do {
        ...
        for (std::array<std::array<tiny, 9>, 27>::const_iterator unit_it =
                 units.begin();
            ...
            for (tiny val = 1; val != 10; ++val) {
                ...
                for (std::array<tiny, 9>::const_iterator cell_it =
                         unit_it->begin();
                     cell_it != unit_it->end(); ++cell_it) {

                    cell = &(sudoku[*cell_it]);

                    if (std::find(cell->begin(), cell->end(), val) !=
                        cell->end())
                        cells.push_back(std::pair<tiny, tiny>(*cell_it, val));
                }

                if (cells.size() == 1 &&
                    sudoku[cells.begin()->first].size() != 1) {
...
void init(void) {
    ...
    for (tiny i = 0; i != 9; ++i) {
        ...
        std::array<tiny, 9> sq_unit = {
            (tiny)(sq_first),      (tiny)(sq_first + 1),
            (tiny)(sq_first + 2),  (tiny)(sq_first + 9),
            (tiny)(sq_first + 10), (tiny)(sq_first + 11),
            (tiny)(sq_first + 18), (tiny)(sq_first + 19),
            (tiny)(sq_first + 20)};
        units[i + 18] = sq_unit;
    }
    ...
    for (tiny cell_i = 0; cell_i != 81; ++cell_i) {
        ...
        for (std::array<std::array<tiny, 9>, 27>::const_iterator unit_it =
            ...
            if (std::find(unit_it->begin(), unit_it->end(), cell_i) !=
                unit_it->end()) {
